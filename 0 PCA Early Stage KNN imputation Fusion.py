...
# Adding prefixes to feature columns to ensure unique feature names
...
# Step for KNN Imputation to handle NaNs
...
n_components = 40  # Set the number of PCA components you want
pca = PCA(n_components=n_components)
features_pca = pd.DataFrame(pca.fit_transform(features_standardized), columns=[f'PC{i+1}' for i in range(n_components)])
...
for run in range(num_runs):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        features_pca, labels, test_size=0.2, random_state=random_seed + run, shuffle=True
    )
...
# Save the PCA-transformed data with the labels
final_data_with_labels = pd.concat([features_pca, labels.reset_index(drop=True)], axis=1)
final_data_with_labels.to_csv('PCA_transformed_with_labels.csv', index=False)
